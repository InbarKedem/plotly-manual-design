...
import type { SeriesConfig, PlotConfig, InteractionConfig, ProgressConfig, ThemeConfig } from "./EnhancedGenericPlotter";
  ...
  const series: SeriesConfig[] = useMemo(() => [
    {
      ...
      data: Array.from({ length: 200 }, (_, i) => ({
        x: i * 0.5,
        y: Math.sin(i * 0.1) * 30 + Math.cos(i * 0.05) * 15 + Math.random() * 10,
        z: 20 + Math.random() * 80, // For color mapping
        text: `Point ${i + 1}<br>Value: ${(Math.sin(i * 0.1) * 30).toFixed(2)}`,
      ...
      hovertemplate: "<b>%{text}</b><br>X: %{x:.2f}<br>Y: %{y:.2f}<br>Z: %{marker.color:.1f}<extra></extra>"
    }
  ], []);
...
export const MultiTypeSeriesExample: React.FC = () => {
  const series: SeriesConfig[] = useMemo(() => [
    ...
    // Scatter series
    {
      ...
      mode: "markers",
      marker: {
        size: [8, 12, 6, 10, 9, 14, 7, 11, 13, 8, 9, 12, 6, 15, 8, 10, 7, 9, 11, 13, 8, 12, 6, 14, 9, 10, 7, 11, 13, 8],
        symbol: "diamond",
        colorFeature: "z",
        colorScale: "plasma",
        opacity: 0.8
  ...
  const series: SeriesConfig[] = useMemo(() => [
    {
      ...
      data: Array.from({ length: 100 }, (_, i) => ({
        x: i,
        y: 50 + Math.sin(i * 0.2) * 20 + Math.cos(i * 0.1) * 10 + Math.random() * 5,
      ...
      hovertemplate: "<b>%{text}</b><br>X: %{x}<br>Y: %{y:.2f}<br>Z: %{marker.color:.1f}<extra></extra>"
    }
  ], []);
...
export const AdvancedClimateExample: React.FC = () => {
  const generateClimateData = (name: string, baseTemp: number, variation: number, points: number) => {
    return Array.from({ length: points }, (_, i) => {
      const altitude = i * 50; // 0-25000m
      const temp = baseTemp - (altitude * 0.0065) + (Math.random() - 0.5) * variation;
      const pressure = 1013.25 * Math.pow(1 - 0.0065 * altitude / 288.15, 5.255);
      const humidity = Math.max(0, Math.min(100, 80 - altitude * 0.001 + Math.random() * 20));
      ...
      return {
        ...
        text: `${name}<br>Alt: ${altitude}m<br>Temp: ${temp.toFixed(1)}°C<br>Pressure: ${pressure.toFixed(1)} hPa`
      };
    });
  };
