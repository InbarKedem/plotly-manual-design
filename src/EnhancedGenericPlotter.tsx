...
export interface ProgressConfig {
  ...
  onProgress?: (progress: number, phase: string, pointsLoaded: number, stats?: DataStats) => void;
...
const MODERN_COLORSCALES: Record<string, [number, string][]> = {
  viridis: [[0, "#440154"], [0.25, "#31688e"], [0.5, "#35b779"], [0.75, "#fde725"], [1, "#fde725"]],
  plasma: [[0, "#0d0887"], [0.25, "#7e03a8"], [0.5, "#cc4778"], [0.75, "#f89441"], [1, "#f0f921"]],
  turbo: [[0, "#23171b"], [0.25, "#1e6091"], [0.5, "#00a76c"], [0.75, "#bfbc00"], [1, "#b30000"]],
  cividis: [[0, "#00224e"], [0.25, "#123570"], [0.5, "#3b496c"], [0.75, "#575d6d"], [1, "#ffea46"]],
  rainbow: [[0, "#ff0000"], [0.17, "#ff8c00"], [0.33, "#ffd700"], [0.5, "#00ff00"], [0.67, "#0000ff"], [0.83, "#4b0082"], [1, "#9400d3"]]
};
...
const GenericPlotter: React.FC<GenericPlotterProps> = ({
  ...
  const plotConfig = useMemo(() => ({
    ...
    xAxis: {
      ...
      gridcolor: config.xAxis?.gridcolor || (theme?.darkMode ? "#444" : "#e1e4e8"),
      zeroline: config.xAxis?.zeroline ?? true,
      zerolinecolor: config.xAxis?.zerolinecolor || (theme?.darkMode ? "#666" : "#d1d5da"),
      ...config.xAxis
    },
    yAxis: {
      ...
      gridcolor: config.yAxis?.gridcolor || (theme?.darkMode ? "#444" : "#e1e4e8"),
      zeroline: config.yAxis?.zeroline ?? true,
      zerolinecolor: config.yAxis?.zerolinecolor || (theme?.darkMode ? "#666" : "#d1d5da"),
      ...config.yAxis
    },
    ...
    legendPosition: config.legendPosition || { x: 1.02, y: 1.0, xanchor: "left", yanchor: "top" },
    margin: config.margin || { l: 60, r: 180, t: 60, b: 60 },
    responsive: config.responsive ?? true,
    useResizeHandler: config.useResizeHandler ?? true,
    backgroundColor: config.backgroundColor || (theme?.darkMode ? "#1a1a1a" : "#ffffff"),
    plotBackgroundColor: config.plotBackgroundColor || (theme?.darkMode ? "#2d3748" : "#fafbfc"),
    font: {
      family: config.font?.family || "Inter, -apple-system, BlinkMacSystemFont, sans-serif",
  ...
  const createTraces = useCallback((seriesConfig: SeriesConfig, seriesIndex: number) => {
    const {
      name,
      data,
      type = "scatter",
    ...
    if (marker.colorFeature) {
      colorValues = data.map(point => getColorValue(point, marker.colorFeature!));
    ...
    if (connectDots && gradientLines && data.length > 1 && type === "scatter") {
      for (let i = 0; i < data.length - 1; i++) {
        const currentValue = colorValues[i] || 0;
        const nextValue = colorValues[i + 1] || 0;
        const avgValue = (currentValue + nextValue) / 2;
        
        let segmentColor = line.color || `hsla(${200 + seriesIndex * 60}, 70%, 50%, 0.8)`;
        if (marker.colorFeature && marker.colorScale) {
          const normalizedValue = (avgValue - colorMin) / (colorMax - colorMin);
          if (typeof marker.colorScale === "string" && MODERN_COLORSCALES[marker.colorScale as keyof typeof MODERN_COLORSCALES]) {
            const scale = MODERN_COLORSCALES[marker.colorScale as keyof typeof MODERN_COLORSCALES];
    ...
    if (mode.includes("lines") && !(gradientLines && connectDots)) {
      mainTrace.line = {
        width: line.width || 2,
        color: line.color || `hsla(${200 + seriesIndex * 60}, 70%, 50%, ${line.opacity || 0.8})`,
        dash: line.dash || "solid",
    ...
    if (mode.includes("markers")) {
      ...
      if (marker.colorFeature && colorValues.length > 0) {
        ...
        if (typeof marker.colorScale === "string" && MODERN_COLORSCALES[marker.colorScale as keyof typeof MODERN_COLORSCALES]) {
          markerConfig.colorscale = MODERN_COLORSCALES[marker.colorScale as keyof typeof MODERN_COLORSCALES];
        } else {
        ...
        markerConfig.color = line.color || `hsla(${200 + seriesIndex * 60}, 70%, 50%, 0.8)`;
      }
  ...
  const getColorFromScale = (value: number, scale: Array<[number, string]>): string => {
    const clampedValue = clamp(value, 0, 1);
    
    for (let i = 0; i < scale.length - 1; i++) {
      if (clampedValue >= scale[i][0] && clampedValue <= scale[i + 1][0]) {
        const factor = (clampedValue - scale[i][0]) / (scale[i + 1][0] - scale[i][0]);
  ...
  const loadDataProgressively = useCallback(async () => {
    ...
    for (let seriesIndex = 0; seriesIndex < series.length; seriesIndex++) {
      ...
      for (let startIndex = 0; startIndex < currentSeries.data.length; startIndex += chunkSize) {
        const endIndex = Math.min(startIndex + chunkSize, currentSeries.data.length);
        ...
        const newTotalPoints = chunkedData.length + processedSeries.reduce((sum, s) => sum + s.data.length, 0);
        ...
        if (progressiveLoading.onProgress) {
          progressiveLoading.onProgress(newProgress, currentPhase, newTotalPoints, stats);
        }
    ...
    setTimeout(() => {
      ...
      setIsComplete(true);
      setCurrentPhase(`Complete - ${stats.totalPoints.toLocaleString()} points loaded`);
  ...
  const debugPanel = debug && dataStats && (
    <div style={{
      ...
      <div>X Range: [{dataStats.xRange[0].toFixed(2)}, {dataStats.xRange[1].toFixed(2)}]</div>
      <div>Y Range: [{dataStats.yRange[0].toFixed(2)}, {dataStats.yRange[1].toFixed(2)}]</div>
      {dataStats.zRange && <div>Z Range: [{dataStats.zRange[0].toFixed(2)}, {dataStats.zRange[1].toFixed(2)}]</div>}
  ...
  return (
    ...
    >
      ...
      {progressiveLoading?.enabled && progressiveLoading.showProgress && isGenerating && (
        <div
          style={{
            position: "absolute",
            top: "20px",
        ...
        >
          ...
          {progressiveLoading.showPhase && (
            <span style={{ fontSize: "14px", fontWeight: "600", color: "#22c55e", textAlign: "center" }}>
              {currentPhase}
            </span>
          )}
