...
export const generateClimateData = (
  ...
  const { bezier, baseSpeed = 50, speedVariation = 30, temperatureRange = [0, 30], pressureRange = [1000, 1020] } = config;
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    const speed = Math.max(15, Math.min(130, baseSpeed + speedNoise - fuelInfluence * 20));

    // Generate temperature with altitude correlation
    const altitude = x;
    const baseTemp = temperatureRange[0] + (temperatureRange[1] - temperatureRange[0]) * Math.random();
    ...
    const basePressure = pressureRange[0] + (pressureRange[1] - pressureRange[0]) * Math.random();
    const pressureAltitudeEffect = basePressure * Math.pow(1 - 0.0065 * altitude / 288.15, 5.255);
    const pressure = Math.max(300, pressureAltitudeEffect + (Math.random() - 0.5) * 50);
...
export const generateSineWave = (
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    const y = amplitude * Math.sin(frequency * x + phase) + (Math.random() - 0.5) * noise;
...
export const generateAtmosphericProfile = (
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    const temperature = seaLevelTemp - lapseRate * altitude + (Math.random() - 0.5) * 5;
    
    // Pressure calculation
    const seaLevelPressure = 1013.25; // hPa
    const pressure = seaLevelPressure * Math.pow(1 - lapseRate * altitude / 288.15, 5.255);
...
export const addNoise = (data: DataPoint[], noiseLevel: number): DataPoint[] => {
...
export const resampleData = (data: DataPoint[], newSize: number): DataPoint[] => {
...
export const normalizeData = (data: DataPoint[], property: keyof DataPoint = 'y'): DataPoint[] => {
  const values = data.map(d => d[property] as number).filter(v => typeof v === 'number');
  ...
  return data.map(point => ({
    ...point,
    [property]: typeof point[property] === 'number' ? ((point[property] as number) - min) / range : point[property],
  }));
};
